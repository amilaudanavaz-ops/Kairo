<script lang="ts">
  import { Calendar, CheckSquare, Clock, Trash2 } from 'lucide-svelte';
...
<aside class="w-[340px] h-full bg-[#111111] flex flex-col shrink-0">
      ...
      {#each plannerStore.inboxBlocks as block (block.id)}
        ...
        >
          <div class="flex items-start justify-between gap-2">
            ...
            <!-- Empty Circle Checkbox (UI only) -->
            <div class="w-3.5 h-3.5 rounded-full border border-zinc-500 shrink-0 mt-0.5"></div>
            
            <div class="flex flex-col gap-1.5 min-w-0">
              <div class="flex items-center gap-2">
                {#if block.type === 'calendar_event'}
                  <span class="w-2 h-2 rounded-full shrink-0" style="background-color: {block.colorHex || '#3b82f6'};"></span>
                {/if}
                <span class="text-sm font-medium text-zinc-200 truncate leading-tight">{block.title}</span>
              </div>
          ...
          {#if block.type !== 'calendar_event'}
            <button 
              onclick={(e) => { e.stopPropagation(); plannerStore.deleteCustomTask(block.id); }}
              class="opacity-0 group-hover:opacity-100 p-1.5 text-zinc-500 hover:text-rose-400 hover:bg-rose-400/10 rounded transition-all cursor-pointer shrink-0"
              title="Delete task"
            >
  ...
  <!-- Exit KFlow Toggle -->
  <div class="p-4 border-t border-[#1e1e1e] bg-[#111111]">
    <button 
      onclick={() => {
